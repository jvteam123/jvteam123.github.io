...
<p style="color:red;">CRITICAL ERROR: Could not connect to Firebase. App will not function correctly. Error: ${error.message}</p>
`;
} else {
...
if (batches.length === 0) {
tlDashboardContentElement.innerHTML = "
<p>No project batches found for TL dashboard.</p>
";
return;
}
...
<tr>
   <td colspan="'+NUM_TABLE_COLUMNS+'" style="color:red; text-align:center;">Error loading projects.</td>
</tr>
';
}
}
...
allowedEmailsList.innerHTML = "
<li>No allowed emails configured. Please add at least one.</li>
";
hideLoading(); return;
}
...
tlSummaryContent.innerHTML = "
<p>Loading summary...</p>
";
if (!db) { tlSummaryContent.innerHTML = '
<p style="color:red;">Database not initialized. Cannot generate summary.</p>
'; hideLoading(); return; }
try {
const projectsSnapshot = await db.collection("projects").get();
let allProjectsData = [];
...
<ul style="list-style: none; padding: 0;">
   ...
   summaryHtml += "
   <h3>Overall Project Totals (All Fix Categories)</h3>
   ";
   ...
   summaryHtml += "
   <h3>Totals by Project and Fix Category</h3>
   ";
   ...
   if (sortedFixCatKeys.length === 0 && sortedOverallKeys.length === 0) { summaryHtml = "
   <p>No project time data found to generate a summary.</p>
   "; }
   else { summaryHtml += "
</ul>
"; }
...
tlSummaryContent.innerHTML = `
<p style="color:red;">Error generating summary: ${error.message}</p>
`;
...
loadingAuthMessageDiv.innerHTML = "
...
<p style="color:red; font-weight:bold;">UI Error: Critical elements missing. Please refresh.</p>
';
...
appContentDiv.style.display = 'none';
loadingAuthMessageDiv.innerHTML = "
<p>Access Denied. Error during sign out. Please refresh.</p>
";
loadingAuthMessageDiv.style.display = 'block';
if (openSettingsBtn) openSettingsBtn.style.display = 'none';
hideLoading();
...
if (loadingMessageElement) {
loadingMessageElement.innerHTML = '
<p style="color:red; font-weight:bold;">Authentication services could not be loaded. Please check the console and refresh.</p>
';
...
<p style="color:red; font-weight:bold;">Authentication services could not be loaded. Please check console and refresh.</p>
';
